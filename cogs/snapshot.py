...
class Snapshot(commands.Cog):
    ...
    async def save(ctx: main.Context) -> dict:
        ...
        for x in ctx.guild.roles:
            payload["roles"][x.id] = {
                ...
                "position": x.position,
                "name": x.name,
                "display_icon": (await x.display_icon.read()).decode("latin1") if x.display_icon and type(x.display_icon) == discord.Asset else x.display_icon if x.display_icon else None,
            }
        ...
        for x in ctx.guild.channels:
            payload["channels"][x.id] = {
                ...
                "slowmode": x.slowmode_delay if x.type not in [discord.ChannelType.voice, discord.ChannelType.category, discord.ChannelType.stage_voice] else None,
                "nsfw": x.is_nsfw() if x.type not in [discord.ChannelType.voice, discord.ChannelType.category, discord.ChannelType.stage_voice] else None,
                ...
                "default_auto_archive_duration": x.default_auto_archive_duration if x.type in [discord.ChannelType.text, discord.ChannelType.forum] else 0,
    ...
    async def create_snapshot(self, ctx: main.Context) -> Optional[UUID]:
        ...
        await self.connection.execute('INSERT INTO snapshots(guild_id, name, payload, author_id, date, code) VALUES($1, $2, $3, $4, $5, $6)', ctx.guild.id, await self.custom_response("snapshot.strings.server_snapshot", ctx), json.dumps(payload), ctx.author.id, datetime.datetime.now(), str(code))
    ...
    async def load_snapshot(self, ctx: main.Context, payload: dict):
        for x in sorted(payload["roles"], key=lambda x: payload["roles"][x]['position'], reverse=True):
            ...
            if not payload["roles"][x]['name'] == "@everyone":
                try:
                    dicon = payload["roles"][x]['display_icon'].encode('latin1') if payload["roles"][x]['display_icon'] else None
                except:
                    dicon = payload["roles"][x]['display_icon'] if payload["roles"][x]['display_icon'] else None
                role = await ctx.guild.create_role(name=payload["roles"][x]['name'], permissions=perms, colour=color, hoist=bool(payload["roles"][x]['hoist']), reason=await self.custom_response("snapshot.strings.save_load_reason", ctx), display_icon=dicon if 'ROLE_ICONS' in ctx.guild.features else None)
                await asyncio.sleep(0.5)
            ...
            if x["type"] == "text" or x["type"] == "news":
                try:
                    ...
                    for z in x["overwrites"]:
                        ...
                        if role:
                            overwrites[role] = discord.PermissionOverwrite.from_pair(discord.Permissions(x["overwrites"][z]["allow"]), discord.Permissions(x["overwrites"][z]["deny"]))
                    await ctx.guild.create_text_channel(name=x['name'], category=cat if cat else None, position=int(x["position"]), reason=await self.custom_response("snapshot.strings.save_load_reason", ctx), slowmode_delay=int(x['slowmode']) if x['slowmode'] else None, topic=x['topic'] if x['topic'] else None, nsfw=bool(x['nsfw']), overwrites=overwrites, news=x["type"] == "news", default_auto_archive_duration=x["default_auto_archive_duration"])
                    await asyncio.sleep(0.5)
                except:
                    continue
            elif x["type"] == "voice":
                try:
                    ...
                    for z in x["overwrites"]:
                        ...
                        if role:
                            overwrites[role] = discord.PermissionOverwrite.from_pair(discord.Permissions(x["overwrites"][z]["allow"]), discord.Permissions(x["overwrites"][z]["deny"]))
                    await ctx.guild.create_voice_channel(name=x['name'], category=cat if cat else None, position=int(x["position"]), reason=await self.custom_response("snapshot.strings.save_load_reason", ctx), bitrate=int(x['bitrate']) if x['bitrate'] else None, user_limit=int(x['user_limit']) if x['user_limit'] else None, overwrites=overwrites, rtc_region=x["rtc_region"])
                    await asyncio.sleep(0.5)
                except:
                    continue
                ...
                try:
                    ...
                    for z in x["overwrites"]:
                        ...
                        if role:
                            overwrites[role] = discord.PermissionOverwrite.from_pair(discord.Permissions(x["overwrites"][z]["allow"]), discord.Permissions(x["overwrites"][z]["deny"]))
                    await ctx.guild.create_stage_channel(name=x['name'], category=cat if cat else None, position=int(x["position"]), reason=await self.custom_response("snapshot.strings.save_load_reason", ctx), overwrites=overwrites)
                    await asyncio.sleep(0.5)
                except:
                    continue
                ...
                try:
                    ...
                    for z in x["overwrites"]:
                        ...
                        if role:
                            overwrites[role] = discord.PermissionOverwrite.from_pair(discord.Permissions(x["overwrites"][z]["allow"]), discord.Permissions(x["overwrites"][z]["deny"]))
                    await ctx.guild.create_category(name=x['name'], position=int(x["position"]), reason=await self.custom_response("snapshot.strings.save_load_reason", ctx), overwrites=overwrites)
                    await asyncio.sleep(0.5)
                except:
                    continue
            elif x["type"] == "forum":
                try:
                    ...
                    for z in x["overwrites"]:
                        ...
                        if role:
                            overwrites[role] = discord.PermissionOverwrite.from_pair(discord.Permissions(x["overwrites"][z]["allow"]), discord.Permissions(x["overwrites"][z]["deny"]))
                    cat = discord.utils.get(ctx.guild.categories, name=x["category"])
                    await ctx.guild.create_forum(name=x['name'], category=cat if cat else None, position=int(x["position"]), reason=await self.custom_response("snapshot.strings.save_load_reason", ctx), nsfw=bool(x['nsfw']), topic=x['topic'] if x['topic'] else None, default_thread_slowmode_delay=int(x["slowmode"]) if x["slowmode"] else None, overwrites=overwrites, default_auto_archive_duration=x["default_auto_archive_duration"])
                    await asyncio.sleep(0.5)
                except:
                    continue
