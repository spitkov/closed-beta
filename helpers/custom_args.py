...
import datetime
from typing import Union, Optional, Literal, Annotated, Sequence
...
class CustomUser:
    ...
    def from_user(cls, user: discord.User):
        ...
        return cls(
            ...
            global_name=f"{user.name}#{user.discriminator}" if user.discriminator != "0" else (user.global_name or user.name),
...
class CustomMember(CustomUser):
    ...
    def from_user(cls, member: discord.Member):
        return cls(
            _name=f"{member.name}#{member.discriminator}" if member.discriminator else member.name,
            id=member.id,
            _discriminator=member.discriminator,
            global_name=f"{member.name}#{member.discriminator}" if member.discriminator else (member.global_name or member.name),
